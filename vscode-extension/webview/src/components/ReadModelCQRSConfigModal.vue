...
async function loadData() {
  ...
  try {
    ...
    const [eventsRes, propsRes, configRes] = await Promise.all([
      fetch(`${apiBase()}/api/readmodel/${props.readModelId}/cqrs/events`),
      fetch(`${apiBase()}/api/readmodel/${props.readModelId}/properties`),
      fetch(`${apiBase()}/api/readmodel/${props.readModelId}/cqrs`)
    ])
...
async function addOperation() {
  ...
  saving.value = true
  try {
    const response = await fetch(`${apiBase()}/api/readmodel/${props.readModelId}/cqrs/operations`, {
      method: 'POST',
...
async function deleteOperation(operationId, index) {
  ...
  saving.value = true
  try {
    const response = await fetch(`${apiBase()}/api/cqrs/operation/${operationId}`, {
      method: 'DELETE'
    })
...
async function addMapping(operationIndex, sourceType, sourcePropertyId, targetPropertyId, staticValue = null) {
  ...
  saving.value = true
  try {
    const response = await fetch(`${apiBase()}/api/cqrs/operation/${operation.id}/mappings`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    if (response.ok) {
      ...
      const sourceProp = sourceType === 'event' 
...
async function deleteMapping(operationIndex, mappingId, mappingIndex) {
  saving.value = true
  try {
    const response = await fetch(`${apiBase()}/api/cqrs/mapping/${mappingId}`, {
      method: 'DELETE'
    })
...
async function addWhereCondition(operationIndex, targetPropertyId, sourceEventFieldId, operator = '=') {
  ...
  saving.value = true
  try {
    const response = await fetch(`${apiBase()}/api/cqrs/operation/${operation.id}/where`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        target_property_id: targetPropertyId,
...
async function deleteWhereCondition(operationIndex, whereId, whereIndex) {
  saving.value = true
  try {
    const response = await fetch(`${apiBase()}/api/cqrs/where/${whereId}`, {
      method: 'DELETE'
    })
...
function submitMapping(opIndex) {
  ...
  addMapping(
    opIndex,
    form.sourceType,
    form.sourcePropertyId,
    form.targetPropertyId,
    form.staticValue
  )
          ...
          <div v-if="showCqrsConfig" class="section cqrs-section">
            ...
            <div v-if="showAddOperation" class="add-operation-form">
              <div class="form-row">
                ...
                <button class="btn-cancel-form" @click="showAddOperation = false">
                  취소
                </button>
              </div>
            </div>
            ...
            >
              ...
              <div class="mapping-section">
                ...
                <div v-for="(mapping, mapIdx) in op.mappings" :key="mapping.id" class="mapping-row">
                  ...
                  <span class="source-field static" v-else>
                    "{{ mapping.staticValue }}"
                  </span>
...
/* Mapping & WHERE Sections */
.mapping-section, .where-section {
  margin-top: 12px;
}
...
/* Add Mapping/WHERE Forms */
.add-mapping-form, .add-where-form {
  display: flex;
  align-items: center;
  gap: 8px;
  padding: 8px;
...
.field-select, .source-type-select, .operator-select {
  background: #1a1a2e;
  border: 1px solid #3d3d6d;
  color: #c0c0c0;
  padding: 6px 10px;
