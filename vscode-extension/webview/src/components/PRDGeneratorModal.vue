...
import { getApiBaseUrl, isVSCode, saveFilesToWorkspace, onMessage, showMessage } from '../utils/vscode'
...
// Computed: Get frameworks compatible with selected language
const availableFrameworks = computed(() => {
  return techStackOptions.value.frameworks.filter(f => 
    f.languages.includes(config.value.language)
  )
})

// Watch language changes to reset framework if incompatible
...
// Computed: Show messaging explanation based on deployment type
...
// Fetch tech stack options on mount
onMounted(async () => {
  try {
    isLoading.value = true
    const response = await fetch(`${apiBase()}/api/prd/tech-stacks`)
    if (response.ok) {
  ...
  // Listen for VS Code messages (file save completion)
  if (isVSCode()) {
    unsubscribe = onMessage((message) => {
      if (message.type === 'filesSaved') {
        if (message.success) {
          step.value = 3
          isGenerating.value = false
        } else {
          error.value = message.error || 'Failed to save files'
          isGenerating.value = false
        }
      }
    })
...
async function generatePreview() {
  ...
  try {
    ...
    const response = await fetch(`${apiBase()}/api/prd/generate`, {
      method: 'POST',
...
// Download ZIP (web) or Save to Workspace (VS Code)
async function downloadZip() {
  ...
  try {
    ...
    // In VS Code, use generate-files endpoint and save to workspace
    if (isVSCode()) {
      const response = await fetch(`${apiBase()}/api/prd/generate-files`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          node_ids: nodeIds,
      ...
      if (!response.ok) {
        const data = await response.json()
        throw new Error(data.detail || 'Failed to generate files')
      }
      
      const files = await response.json()
      
      // Send files to extension to save to workspace
      saveFilesToWorkspace(files, config.value.project_name)
      
      // The step transition will happen when we receive the filesSaved message
      return
    }
    
    // Web version: download as ZIP
    const response = await fetch(`${apiBase()}/api/prd/download`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        node_ids: nodeIds,
        ...
        <div class="modal-content">
              ...
              <div class="form-grid">
                <div class="form-group">
                  ...
                  <select v-model="config.language" class="form-select">
                    <option 
                      v-for="lang in techStackOptions.languages" 
                      :key="lang.value" 
                      :value="lang.value"
                    >
                      {{ lang.label }}
              ...
              <div class="form-grid">
                <div class="form-group">
                  ...
                  <div class="radio-cards">
                    <label 
                      class="radio-card" 
                      :class="{ selected: config.deployment === 'microservices' }"
                    >
                      <input 
                        type="radio" 
                    ...
                    <label 
                      class="radio-card" 
                      :class="{ selected: config.deployment === 'modular-monolith' }"
                    >
                      <input 
                        type="radio" 
                        v-model="config.deployment" 
              ...
              <div class="form-grid">
                <div class="form-group">
                  ...
                  <select v-model="config.messaging" class="form-select">
                    <option 
                      v-for="msg in techStackOptions.messaging" 
                      :key="msg.value" 
                      :value="msg.value"
                    >
                      {{ msg.label }}
                ...
                <div class="form-group">
                  ...
                  <select v-model="config.database" class="form-select">
                    <option 
                      v-for="db in techStackOptions.databases" 
                      :key="db.value" 
                      :value="db.value"
                    >
                      {{ db.label }}
          ...
          <div v-if="step === 2 && previewData" class="preview-step">
            <div class="preview-section">
              ...
              <div class="bc-list">
                <div 
                  v-for="bc in previewData.bounded_contexts" 
                  :key="bc.id" 
                  class="bc-item"
                >
            ...
            <div class="preview-section">
              ...
              <div class="file-tree">
                <div 
                  v-for="file in previewData.files_to_generate" 
                  :key="file" 
                  class="file-item"
                >
            ...
            <div class="preview-section">
              ...
              <div class="tech-summary">
                <div class="tech-item">
                  ...
                  <span class="tech-value">{{ previewData.tech_stack.language }}</span>
                </div>
                <div class="tech-item">
                  <span class="tech-label">Framework:</span>
                  <span class="tech-value">{{ previewData.tech_stack.framework }}</span>
                </div>
                <div class="tech-item">
                  <span class="tech-label">Messaging:</span>
                  <span class="tech-value">{{ previewData.tech_stack.messaging }}</span>
                </div>
                <div class="tech-item">
                  <span class="tech-label">Deployment:</span>
                  <span class="tech-value">{{ previewData.tech_stack.deployment }}</span>
                </div>
              </div>
            </div>
          </div>
...
.btn-primary {
  background: linear-gradient(135deg, #7aa2f7, #5d8ffc);
  color: white;
}

.btn-primary:hover:not(:disabled) {
  background: linear-gradient(135deg, #8aafff, #6d9aff);
