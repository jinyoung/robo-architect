...
const emit = defineEmits(['preview-ui'])
...
// Double-click to open UI preview
function handleDoubleClick() {
  emit('preview-ui', {
    id: props.id,
    name: props.data.name,
    template: props.data.template,
    attachedToId: props.data.attachedToId,
    attachedToName: props.data.attachedToName,
    attachedToType: props.data.attachedToType,
    userStoryId: props.data.userStoryId
  })
}
</script>
...
<template>
  <div 
    class="es-node es-node--ui"
    :class="{ 'has-template': hasTemplate }"
...
.es-node--ui:hover .es-node__hint {
  opacity: 1;
}
